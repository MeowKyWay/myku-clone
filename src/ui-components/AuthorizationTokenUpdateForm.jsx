...
import { updateAuthorizationToken } from "../graphql/mutations";
const client = generateClient();
export default function AuthorizationTokenUpdateForm(props) {
  const {
    id: idProp,
    authorizationToken: authorizationTokenModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const [authorizationTokenRecord, setAuthorizationTokenRecord] =
    React.useState(authorizationTokenModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getAuthorizationToken.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getAuthorizationToken
        : authorizationTokenModelProp;
      setAuthorizationTokenRecord(record);
    };
    queryData();
  }, [idProp, authorizationTokenModelProp]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateAuthorizationToken.replaceAll("__typename", ""),
            variables: {
              input: {
                id: authorizationTokenRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "AuthorizationTokenUpdateForm")}
      {...rest}
    >
      <TextField
        label="Token"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || authorizationTokenModelProp)}
